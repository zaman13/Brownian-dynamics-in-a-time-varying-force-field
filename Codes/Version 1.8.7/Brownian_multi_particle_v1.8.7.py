...
def anim_setup():
    ...
    ax2.set_xlim(y_lim[0], y_lim[1])
    ax2.set_ylim(z_lim[0], z_lim[1])
    ...
    for p in range(Np):
         cl_p = particle_color(p)
         
                 
         beads_xy.append(plt.patches.Circle((r_um[0,p,0], r_um[1,p,0]), ro[p]*1e6, fc=cl_p,ec = 'k',zorder = 10))
         beads_yz.append(plt.patches.Circle((r_um[1,p,0], r_um[2,p,0]), ro[p]*1e6, fc=cl_p,ec = 'k',zorder = 10))
         beads_xz.append(plt.patches.Circle((r_um[0,p,0], r_um[2,p,0]), ro[p]*1e6, fc=cl_p,ec = 'k',zorder = 10))
...
for m in range(Nt-1):
    ...
    v_fluid   = fluid_vel(r[:,:,m],t[m])
    
    
    # v_fluid_noisy = np.random.normal(1,.01,(3,Np))*v_fluid
    # v_temp = (Lambda/(1+Lambda)) * v[:,:,m] + (1/(1+Lambda)) * ( v_fluid_noisy + fct_1 * w[:,:,m]  +  fct_2 * force_ext )
    ...
    # v_temp = v_temp*(-1.0)
    v_temp = particle_collision_adjust(r[:,:,m], v_temp, ro, mo, damping_factor) 
    # v[:,:,m+1] = wall_collision_adjust(r[:,:,m], v_temp, ro, damping_factor, p_wall_set, d_wall_set, lim_wall_set)
    v[:,:,m+1] = wall_collision_adjust(r[:,:,m], v_temp, ro, damping_factor, n_vector_set, d_set, p_lim_set)
    # v[:,:,m+1] = v_temp
...
# May 27, 2021
# Plot 4 particle data or less (with large number of particles, plotting all of them becomes crowded)
for p in range(min(Np,4)):
...
anim.save('br_v1.8.mp4', fps=frame_rate, extra_args=['-vcodec', 'libx264'])
