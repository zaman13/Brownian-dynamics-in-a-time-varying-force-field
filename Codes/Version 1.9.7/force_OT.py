...
def force_profile(r_in, t):
    ...
    # Nov. 1, 2024: Adding a second light ON event    
    if (t > 15) & (t<23):
        xshift = 10e-6
        yshift = 10e-6
        fm[0,:] = -2*n_order*(r_in[0,:]-xshift)/(r_norm**2) * (r_norm/w_well)**(2*n_order) * g *(ro)**3
        fm[1,:] = -2*n_order*(r_in[1,:]-yshift)/(r_norm**2) * (r_norm/w_well)**(2*n_order) * g *(ro)**3
        fm[2,:] = -2*n_order*(r_in[2,:])/(r_norm**2) * (r_norm/w_well)**(2*n_order) * g *(ro)**3
