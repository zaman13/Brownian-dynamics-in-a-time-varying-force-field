...
Input arguments:
    ...
    - p_wall_set is a (3,Nwall) array of vectors (one 3 vector per wall)
      Each reflecting wall is defined by a plane with equation ax + by + cz = d
      The p_wall_set vectors denote the coefficients a,b,c of the planes
...
def wall_collision_detect(r_in,ro,p_wall,d_wall,lim_wall):
    ...
    ind2 = np.where((lim_wall[0]-ro <= r_in[0,:]) & (r_in[0,:] <= lim_wall[1]+ro) & (lim_wall[2]-ro <= r_in[1,:]) & (r_in[1,:] <= lim_wall[3]+ro) & (lim_wall[4]-ro <= r_in[2,:]) & (r_in[2,:] <= lim_wall[5]+ro)) 
...
def wall_collision_adjust(r_in,v_in,ro,damping_factor,p_wall_set,d_wall_set, lim_wall_set):
    ...
    Nwall = p_wall_set.shape[0]    # Detect number of walls
    
    # Loop over all walls
    for wall in range(Nwall):
        p_wall = p_wall_set[wall]      # coefficient vector for the wall
        d_wall = d_wall_set[wall]      # offset vector for the wall
        n_wall = p_wall/np.linalg.norm(p_wall)  # normal vector for the wall
        
        lim_wall = lim_wall_set[wall]  # 6 x 1 array containing limits of the walls [xmin, xmax, ymin, ymax, zmin, zmax] format.
...
# p1 = r[:,:,0]
# # v1 = v[:,:,0]
# p_wall_set = [[0,0,1], [0,1,0], [0,1,0] ]                          # plane coefficient vector array
...
# p_wall_set = np.array(p_wall_set)                                  # convert to numpy array
# d_wall_set = np.array(d_wall_set)

# lim_wall_set = np.array(lim_wall_set)
# i1 = wall_collision_detect(p1,ro,p_wall_set[0],d_wall_set[0],lim_wall_set[0])
# # print(i1)
