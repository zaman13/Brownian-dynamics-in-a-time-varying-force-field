...
# p_wall_set = [[0,0,1e-6],[0,1e-6,0]]
# d_wall_set = [5e-6,2e-6]

p_wall_set = [[0,0,1]]                  # plane coefficient vector array
d_wall_set = [0]                        # plane right-hand term array

# Limits on the extent of the walls. Format: [xmin, xmax, ymin, ymax, zmin, zmax] per wall
lim_wall_set = [[-np.inf, np.inf, -np.inf, np.inf, -np.inf, np.inf],     
               ]


p_wall_set = np.array(p_wall_set)
