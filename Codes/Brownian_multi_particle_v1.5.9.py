...
for m in range(Nt-1):
    ...
    v_temp = 1/(1+fct2)*(fct2*v[:,:,m] + force_temp/gamma + 0*v_br[:,:,m])
    # v_temp = v_temp*(-1.0)
...
draw_electrodes(elec_width, elec_spacing)
...
anim.save('br_v1.4.mp4', fps=frame_rate, extra_args=['-vcodec', 'libx264'])
